    ...
    <div className="w-80 border-r h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        {filtered.length === 0 ? (
          ...
          filtered.map((c) => {
            const chat = chatMap[c._id]; // if chat exists
            ...
            return (
              <div
                key={c._id}
                onClick={
                  () => setActiveChat(chat ? chat.chatId : c._id) // if no chat => use userId
                }
                className={`p-3 flex gap-3 cursor-pointer hover:bg-gray-100 ${
                  activeChat === (chat ? chat.chatId : c._id)
                    ? "bg-gray-100"
                    : ""
                }`}
              >
